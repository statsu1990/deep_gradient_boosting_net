...
class DeepGBnet:
    ...
    def train_boosting_model(self, x_tr, y_tr, x_ts, y_ts, epochs, batch_size, do_subsampling=True, save_result_dir=None):
        ...
        for iboost in range(self.BOOSTING_NUM):
            ...
            # built temp model
            new_weak_model = self.build_weak_model()
            if boosting_model is not None:
                # freeze
                boosting_model.trainable = False
            temp_classify_model = self.build_classify_model(
                    boosting_model=self.build_boosting_model(boosting_model, new_weak_model, do_shrink=False)
                    )
            
            # train temp_classify_model
            opt = keras.optimizers.Nadam(lr=0.002, beta_1=0.9, beta_2=0.999, epsilon=None, schedule_decay=0.004)
            temp_classify_model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'])
            temp_classify_model.fit_generator(self.datagen.flow(x_subsamp, y_subsamp, batch_size=batch_size),
            ...
            # score
            # train
            scores = temp_classify_model.evaluate(x_tr, y_tr, verbose=0)
            self.train_scores.append([scores[0], scores[1]])
            print('\nTrain loss, accuracy: {0:.4f}, {1:.4f}'.format(scores[0], scores[1]))
            # test
            scores = temp_classify_model.evaluate(x_ts, y_ts, verbose=0)
            self.test_scores.append([scores[0], scores[1]])
            print('Test loss, accuracy: {0:.4f}, {1:.4f}'.format(scores[0], scores[1]))

            # make boosting model
            boosting_model = self.build_boosting_model(boosting_model, new_weak_model, do_shrink=True)
            

        # make classify model
        self.classify_model = self.build_classify_model(boosting_model)
        self.classify_model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'])

        if save_result_dir is not None:
            self.save_plot_model(new_weak_model, os.path.join(save_result_dir, 'weak_model_structure.png'))
            self.save_plot_model(boosting_model, os.path.join(save_result_dir, 'boostring_model_structure.png'))

        return

    def train_boosting_model2(self, x_tr, y_tr, x_ts, y_ts, epochs, batch_size, do_subsampling=True, save_result_dir=None):
        ...
        for iboost in range(self.BOOSTING_NUM):
            ...
            if boosting_model is not None:
                # freeze
                boosting_model.trainable = False
            temp_boosting_model = self.build_boosting_model(boosting_model, new_weak_model, do_shrink=False)
            temp_classify_model = self.build_classify_model(temp_boosting_model)
            ...
            # score
            # train
            scores = self.classify_model.evaluate(x_tr, y_tr, verbose=0)
            self.train_scores.append([scores[0], scores[1]])
            print('\nTrain loss, accuracy: {0:.4f}, {1:.4f}'.format(scores[0], scores[1]))
            # test
            scores = self.classify_model.evaluate(x_ts, y_ts, verbose=0)
            self.test_scores.append([scores[0], scores[1]])
            print('Test loss, accuracy: {0:.4f}, {1:.4f}'.format(scores[0], scores[1]))
    ...
    def build_weak_model(self):
        ...
        h = Activation('relu')(h)
        h = Conv2D(32, (3, 3), kernel_initializer='he_normal')(h)
        h = BatchNormalization()(h)
        h = Activation('relu')(h)
        ...
        h = BatchNormalization()(h)
        h = Activation('relu')(h)
        h = Conv2D(64, (3, 3), kernel_initializer='he_normal')(h)
        h = BatchNormalization()(h)
        h = Activation('relu')(h)
        h = MaxPooling2D(pool_size=(2, 2))(h)
        ...
        # fully conection
        h = Dense(10, kernel_regularizer=keras.regularizers.l2(self.L2), kernel_initializer='he_normal')(h)
        h = BatchNormalization()(h)
        h = Activation('relu')(h)
    ...
    def build_boosting_model(self, boosting_model, weak_model, do_shrink):
        # input
        input_img = Input(self.INPUT_SHAPE)        
        # boost model output
        if boosting_model is not None:
            boost_oup = boosting_model(input_img)
        # weak model output
        weak_oup = weak_model(input_img)
        if do_shrink:
            shrinkage = self.SHRINKAGE
            weak_oup = Lambda(lambda x: x * shrinkage, output_shape=(self.OUP_CLASS_NUM,))(weak_oup)
        # add
        if boosting_model is not None:
            add_oup = Add()([boost_oup, weak_oup])
        else:
            add_oup = weak_oup

        model = Model(inputs=input_img, outputs=add_oup)

        return model

    def build_classify_model(self, boosting_model):
        """
        ...
        boost_oup = boosting_model(input_img)
        # output
        oup = Activation('softmax')(boost_oup)
    ...
    def subsumpling_index(self, index_num):
        ...
        bagging
        """
        subsamp_idx = np.random.choice(np.arange(index_num), size=index_num, replace=True)
        return subsamp_idx
